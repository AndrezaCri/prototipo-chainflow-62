...
export const SwapModal: React.FC<SwapModalProps> = ({
    ...
    const initializeService = async () => {
      if (isConnected && walletClient) {
        try {
          ...
          const provider = new ethers.providers.Web3Provider(walletClient as any, 'any');
  ...
  const loadBalances = async () => {
    ...
    try {
      const [usdcBalance, brzBalance] = await Promise.all([usdcBrzSwapService.getTokenBalance('USDC'), usdcBrzSwapService.getTokenBalance('BRZ')]);
      if (tokenIn === 'USDC') {
  ...
  const executeSwap = async () => {
    ...
    try {
      ...
      const transaction = await usdcBrzSwapService.executeSwap(amountIn, tokenIn, tokenOut, slippage);
  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          {!isConnected && <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          ...
          {isConnected && showTestFaucet && <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg space-y-3">
              ...
              <Button onClick={handleGetTestTokens} disabled={balanceLoading} size="sm" className="bg-blue-600 hover:bg-blue-700 text-white">
                {balanceLoading ? <>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <div className="flex-1 relative">
                <Input type="number" placeholder="0.00" value={amountIn} onChange={e => setAmountIn(e.target.value)} className="pr-16" />
                <button onClick={handleMaxAmount} disabled={!isConnected || !balanceIn || parseFloat(balanceIn) === 0} className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-[#c1e428] hover:text-[#a8c424] font-medium disabled:text-gray-400 disabled:cursor-not-allowed">
                  MAX
                </button>
              </div>
          ...
          <div className="flex justify-center">
            <button onClick={handleSwapTokens} className="p-2 rounded-full border-2 border-gray-200 hover:border-[#c1e428] transition-colors">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <div className="flex-1 relative">
                <Input type="number" placeholder="0.00" value={amountOut} readOnly className="bg-gray-50" />
          ...
          {/* Detalhes da cotação */}
          {quote && <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg space-y-2 text-sm">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {[0.1, 0.5, 1.0].map(value => <button key={value} onClick={() => setSlippage(value)} className={`px-3 py-1 text-sm rounded ${slippage === value ? 'bg-[#c1e428] text-black' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                  {value}%
                </button>)}
              <Input type="number" step="0.1" min="0.1" max="50" value={slippage} onChange={e => setSlippage(parseFloat(e.target.value) || 0.5)} className="w-20 text-sm" />
            </div>
          </div>

          {/* Botão de swap */}
          <Button onClick={executeSwap} disabled={!isConnected || !quote || swapping || parseFloat(amountIn || '0') <= 0 || showTestFaucet} className="w-full bg-[#c1e428] hover:bg-[#a8c424] text-black font-medium">
            {!isConnected ? 'Conecte sua carteira' : showTestFaucet ? 'Adicione tokens de teste primeiro' : swapping ? <>
